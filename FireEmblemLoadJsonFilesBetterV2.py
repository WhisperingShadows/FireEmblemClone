...
def load_files(skill_class, player_class, enemy_class, weapon_class, output_as_class=True, get_english_data=True,
               ...
               get_stage_encount=True, get_terrain=True, check_for_update=False, get_simple_names=False):
    # print("Starting")
    start = time()
    ...
    def my_merger2(list_of_dicts, output_class):
        ...
        for idict in list_of_dicts:
            ...
            if output_class == player_class:
                ...
                simple_name = translate_jp_to_en_dict(idict, english_data, prefix="MPID", old_prefix="PID")
                ...
                if translated_name not in my_dict:
                    ...
                    def get_gender(dictionary: dict):
                        ...
                        if len(gender) == 0:
                            ...
                            raise ValueError(
                                "Face_name of unit {0} contains more than one gender".format(idict["id_num"]))

                    if output_as_class:
                        # creates new entry name that includes id_num in key for old entry
                        old_entry = str(
                            "PID_" +
                            str(weapon_index_dict[my_dict[translated_name].weapon_type]).replace(" ", "_") +
                            "_" +
                            str(move_index_dict[my_dict[translated_name].move_type]) +
                            "_" +
                            str(translated_name).replace("PID_", ""))

                        # creates new entry name that includes id_num in key for duplicate entry
                        duplicate_entry = str(
                            "PID_" +
                            str(weapon_index_dict[idict["weapon_type"]]).replace(" ", "_") +
                            "_" +
                            str(move_index_dict[idict["move_type"]]) +
                            "_" +
                            str(translated_name).replace("PID_", ""))

                        if old_entry == duplicate_entry:
                            old_entry += "_" + get_gender(my_dict[translated_name])
                            duplicate_entry += "_" + get_gender(idict)

                        my_dict[old_entry] = my_dict[translated_name]
                        my_dict[duplicate_entry] = output_class.from_dict(input_dict=idict)
                    else:
                        # creates new entry name that includes id_num in key for old entry
                        old_entry = str(
                            "PID_" +
